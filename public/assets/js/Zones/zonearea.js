...
$(document).ready(function()
{
    ...
    $("#datatable").DataTable(),
       table =  $('#datatable-buttons').DataTable(
            ...
            "processing":true,
            "serverSide":true,
            "ajax":request_url+"loadZonearea",
            "columns":[
                {"data":"area_id"},
                {"data":"zone_name"},
                {"data":"area_name"},
                {"data":"area_delivery_charges"},
                {"data":"area_status"}
            ],
        ...
        $('#datatable-buttons').on('draw.dt',function(){
            $('#datatable-buttons').Tabledit({
                url:request_url+'updateZonerea',
                method:"POST",
                dataType:'json',
                columns:{
                    identifier : [0, 'area_id'],
                    editable:[
                            [2, 'area_name'],
                            [3,'area_delivery_charges'],
                            [4, 'area_status',"{'Active':'Active','Not Active':'Not Active'}"]
                    ]},
